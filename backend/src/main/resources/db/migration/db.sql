...
CREATE TABLE IF NOT EXISTS MODELPRODUCT (
...
CREATE OR REPLACE VIEW MP AS SELECT * FROM MODEL INNER JOIN MODELPRODUCT ON MODEL.MODELID = MODELPRODUCT.FK_MODELID INNER JOIN PRODUCT ON PRODUCT.PRODUCTID = MODELPRODUCT.FK_PRODUCTID;
SELECT * FROM MP;

CREATE OR REPLACE VIEW SP AS SELECT * FROM PRODUCT INNER JOIN STAFFPRODUCT ON STAFFPRODUCT.FK_PRODUCTID = PRODUCT.PRODUCTID INNER JOIN STAFF ON STAFFPRODUCT.FK_STAFFID = STAFF.STAFFID;
SELECT * FROM SP;

CREATE OR REPLACE VIEW MPS AS SELECT MP.MODELID AS M_ID, MP.M_PRODUCTNR, MP.M_WARRANTY, MP.M_LIFESPAN, MP.M_IMAGE, MP.M_TBIMAGE, MP.PRODUCTID AS P_ID, MP.NAME AS P_NAME, MP.TYPE AS P_TYPE, MP.SERIALNR AS P_SERIALNR, MP.STATUS AS P_STATUS, MP.P_WARRANTY, MP.P_LIFESPAN, MP.P_PURCHASED, MP.P_DISPOSED, MP.COMMENT AS P_COMMENT , SP.STAFFID AS S_ID, SP.NAME AS S_NAME FROM MP INNER JOIN SP ON MP.PRODUCTID = SP.PRODUCTID;
